...
uint8 MQTT_UnPacketCmd(uint8 *rev_data, int8 **cmdid, int8 **req)
{
	...
	*cmdid = (int8 *)MQTT_MallocBuffer(37);				//cmdid固定36字节，多分配一个结束符的位置
	...
	strncpy(*cmdid, (const int8 *)dataPtr, 36);			//复制cmdid
	dataPtr += 36;
	...
	*req = (int8 *)MQTT_MallocBuffer(req_len + 1);		//分配命令长度+1
	...
	strncpy(*req, (const int8 *)dataPtr, req_len);		//复制命令
...
/*************************************固定头部***********************************************/
	...
	//固定头部----------------------头部消息-------------------------------------------------
	mqttPacket->_data[mqttPacket->_len++] = MQTT_PKT_SUBSCRIBE << 4;
...
/*************************************固定头部***********************************************/
	...
	//固定头部----------------------头部消息-------------------------------------------------
	mqttPacket->_data[mqttPacket->_len++] = MQTT_PKT_UNSUBSCRIBE << 4;
...
//	说明：		
//==========================================================
uint8 MQTT_UnPacketPublish(uint8 *rev_data, int8 **topic, int8 **payload, uint8 *qos, uint16 *pkt_id)
{
...
/*************************************固定头部***********************************************/
	...
	//固定头部----------------------头部消息-------------------------------------------------
	mqttPacket->_data[mqttPacket->_len++] = MQTT_PKT_PUBREL << 4;
